    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="relative w-full max-w-2xl mx-auto rounded-2xl border border-white/15 bg-neutral-900 shadow-xl p-8">
        ...
        <div className="mb-4">
          ...
          <div>
            {property.addressLine} {property.unit && <span>(Unidad: {property.unit})</span>} {property.city && <span>- {property.city}</span>} {property.province && <span>- {property.province}</span>}
          </div>
        </div>
        <div className="mb-4">
          <div className="text-white font-semibold mb-1">Propietario:</div>
          <div>{typeof property.ownerId === "object" ? property.ownerId.fullName : property.ownerId}</div>
        </div>
        {property.inquilinoId && (
          <div className="mb-4">
            <div className="text-white font-semibold mb-1">Inquilino:</div>
            <div>{typeof property.inquilinoId === "object" ? property.inquilinoId.fullName : property.inquilinoId}</div>
          </div>
        )}
        ...
        <div className="flex flex-col gap-1 text-xs text-neutral-400 mt-4">
          <div>
            <b>Estado:</b> {property.inquilinoId && property.inquilinoId !== "" ? "Alquilado" : "Disponible"}
          </div>
