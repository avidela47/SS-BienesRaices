...
/** ✅ NORMALIZA ids que a veces vienen como ObjectId/objeto */
function asId(v: unknown): string {
  if (typeof v === "string") return v;
  if (typeof v === "number") return String(v);
  if (v && typeof v === "object") {
    const anyObj = v as { toString?: () => string; $oid?: string; oid?: string; _id?: unknown };
    if (typeof anyObj.$oid === "string") return anyObj.$oid;
    if (typeof anyObj.oid === "string") return anyObj.oid;
    if (typeof anyObj.toString === "function") return anyObj.toString();
    if (anyObj._id) return asId(anyObj._id);
  }
  return String(v ?? "");
}
...
export default function PaymentsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (cRes.ok && cData.ok && Array.isArray(cData.contracts)) setContracts(cData.contracts ?? []);
      else setContracts([]);

      if (iRes.ok && iData.ok && Array.isArray(iData.installments)) setInstallments(iData.installments ?? []);
  ...
  const installmentLabelById = useMemo(() => {
    return installments.reduce<Record<string, string>>((acc, it) => {
      acc[asId(it._id)] = it.period || asId(it._id);
      return acc;
    }, {});
  }, [installments]);
  ...
  const filtered = useMemo(() => {
    ...
    return payments.filter((p) => {
      const contractId = asId(p.contractId);

      if (method !== "ALL" && p.method !== method) return false;
      if (contractFilter !== "ALL" && contractId !== contractFilter) return false;

      if (c) {
        const code = (contractCodeById[contractId] || "").toLowerCase();
        if (!contractId.toLowerCase().includes(c) && !code.includes(c)) return false;
      }
  ...
  const balanceByContract = useMemo(() => {
    const totals = installments.reduce<Record<string, number>>((acc, it) => {
      const cid = asId(it.contractId);
      acc[cid] = (acc[cid] || 0) + (Number(it.amount) || 0);
      return acc;
    }, {});

    const paid = payments.reduce<Record<string, number>>((acc, p) => {
      if (p.status === "OK") {
        const cid = asId(p.contractId);
        acc[cid] = (acc[cid] || 0) + (Number(p.amount) || 0);
      }
      return acc;
    }, {});
  ...
  async function confirmEdit() {
    ...
    const pid = asId(editTarget._id);
    if (!pid || pid === "undefined" || pid === "null") {
      setErr("ID inválido (front). Refrescá la página.");
      return;
    }
    ...
    setErr("");
    try {
      const res = await fetch(`/api/payments/${encodeURIComponent(pid)}`, {
        method: "PATCH",
          ...
          <div className="p-4">
            <div className="rounded-xl border border-white/10 overflow-hidden">
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((p) => {
                  const pid = asId(p._id);
                  const contractId = asId(p.contractId);
                  const installmentId = asId(p.installmentId);
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="col-span-2 text-neutral-300">{contractCodeById[contractId] ?? contractId}</div>

                      <div className="col-span-1 text-center">
                        <Link className="text-emerald-300 hover:underline" href={`/installments?focus=${installmentId}`}>
                          <span className="text-xs">{installmentLabelById[installmentId] ?? "—"}</span>
                        </Link>
                      </div>
                      ...
                      <div className="col-span-2 flex justify-end gap-2">
                        <button
                          ...
                          onClick={() => openEdit(p)}
                          disabled={loading || p.status === "VOID" || !pid || pid === "[object Object]"}
            ...
            <div className="w-full max-w-xl rounded-2xl border border-white/10 bg-black/80 shadow-2xl">
              <div className="px-5 py-4 border-b border-white/10">
                ...
                <div className="text-sm text-neutral-400 mt-1">
                  {formatDateTime(editTarget.date)} — {formatARS(editTarget.amount)} —{" "}
                  {contractCodeById[asId(editTarget.contractId)] ?? asId(editTarget.contractId)} —{" "}
                  {installmentLabelById[asId(editTarget.installmentId)] ?? "—"}
                </div>
              </div>
          ...
          <div className="fixed inset-0 z-9998 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="w-full max-w-xl rounded-2xl border border-white/10 bg-black/80 shadow-2xl">
              <div className="px-5 py-4 border-b border-white/10">
                ...
                <div className="text-sm text-neutral-400 mt-1">
                  {formatDateTime(voidTarget.date)} — {formatARS(voidTarget.amount)} — {asId(voidTarget._id)}
                </div>
              </div>
